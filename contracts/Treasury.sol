...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SZD }
    ...
    address public sZD;
    uint public sZDQueue; // Delays change to sZD address
    ...
    /**
        @notice allow approved address to deposit an asset for ZD
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ZD needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ZD ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ZD for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IMushERC20( ZD ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sZD ).balanceOf( msg.sender ); // Can only borrow against sZD held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ZD
        @param _amount uint
     */
    function repayDebtWithZD( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IMushERC20( ZD ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ZD, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ZD ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ZD ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ZD valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ZD decimals
            value_ = _amount.mul( 10 ** IERC20( ZD ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SZD ) { // 9
            sZDQueue = block.number.add( blocksNeededForQueue );
        } else return false;
