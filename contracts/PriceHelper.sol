...
contract BondPriceHelper is Ownable {
    ...
    function viewPriceAdjust(address bond, uint percent)
        view
        external
        returns(uint _controlVar, uint _oldControlVar, uint _minPrice, uint _oldMinPrice, uint _price)
    {
        uint price = IBond(bond).bondPrice();
        (uint controlVariable, , uint minimumPrice,, , ) = IBond(bond).terms();
        if(minimumPrice==0){
            return (
    ...
    function adjustPrice(address bond, uint percent) external {
        ...
        (uint controlVariable, uint vestingTerm, uint minimumPrice,uint maxPayout, uint fee, uint maxDebt) = IBond(bond).terms();
        if(minimumPrice == 0){
