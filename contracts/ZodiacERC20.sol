...
contract ZodiacERC20 is ERC20Permit, VaultOwned {
    using SafeMath for uint256;

    uint256 public antiWhaleTime;
    uint256 public antiWhaleAmount;

    constructor() ERC20("ZodiacDAO Token", "ZD", 9) {}

    function antiWhale(uint256 _amount) external onlyOwner() {
        require(_amount > 0);
        require(antiWhaleTime == 0);

        antiWhaleAmount = _amount;
        antiWhaleTime = block.timestamp.add(15 minutes);
    }
